...
(() => {
    ...
    const QUEUE_TYPES = {
        ...
        quickplay: { id: 490, name: 'Normal (Quickplay)' }
    };
    ...
    const utils = {
        ...
        async retry(fn, retries = CONFIG.retryAttempts) {
            try {
                ...
                if (retries > 0) {
                    await new Promise(resolve => setTimeout(resolve, CONFIG.retryDelay * (CONFIG.retryAttempts - retries + 1)));
    ...
    class ProfileWinLoseStats {
        ...
        initializeSettings() {
            ...
            const addSettings = (scrollable) => {
                if (!scrollable || document.getElementById('profile-winloss-settings')) return;
                    ...
                    <div style="display: flex; flex-direction: column; gap: 15px; margin-top: 10px;">
                        ...
                        <div style="display: flex; align-items: center; gap: 10px; padding-bottom: 10px; border-bottom: thin solid #3c3c41;">
                            ...
                            <lol-uikit-framed-dropdown class="lol-settings-general-dropdown" style="width: 200px;" tabindex="0">
                                <lol-uikit-dropdown-option slot="lol-uikit-dropdown-option" class="framed-dropdown-type" selected="${CONFIG.selectedQueue === 'all'}" value="all">
                                    All Queues
                                    <div class="lol-tooltip-component"></div>
                                </lol-uikit-dropdown-option>
                                <lol-uikit-dropdown-option slot="lol-uikit-dropdown-option" class="framed-dropdown-type" selected="${CONFIG.selectedQueue === 'quickplay'}" value="quickplay">
                                    Normal (Quickplay)
                                    <div class="lol-tooltip-component"></div>
                                </lol-uikit-dropdown-option>
                                <lol-uikit-dropdown-option slot="lol-uikit-dropdown-option" class="framed-dropdown-type" selected="${CONFIG.selectedQueue === 'ranked_solo'}" value="ranked_solo">
                                    Ranked (Solo/Duo)
                                    <div class="lol-tooltip-component"></div>
                                </lol-uikit-dropdown-option>
                                <lol-uikit-dropdown-option slot="lol-uikit-dropdown-option" class="framed-dropdown-type" selected="${CONFIG.selectedQueue === 'ranked_flex'}" value="ranked_flex">
                                    Ranked (Flex)
                                    <div class="lol-tooltip-component"></div>
                                </lol-uikit-dropdown-option>
                                <lol-uikit-dropdown-option slot="lol-uikit-dropdown-option" class="framed-dropdown-type" selected="${CONFIG.selectedQueue === 'normal_draft'}" value="normal_draft">
                                    Normal (Draft Pick)
                                    <div class="lol-tooltip-component"></div>
                                </lol-uikit-dropdown-option>
                                <lol-uikit-dropdown-option slot="lol-uikit-dropdown-option" class="framed-dropdown-type" selected="${CONFIG.selectedQueue === 'aram'}" value="aram">
                                    ARAM
                                    <div class="lol-tooltip-component"></div>
                                </lol-uikit-dropdown-option>
                ...
                const dropdownOptions = dropdown.querySelectorAll('lol-uikit-dropdown-option');
                
                // Set initial selected value
                const currentOption = Array.from(dropdownOptions).find(opt => opt.getAttribute('value') === CONFIG.selectedQueue);
                if (currentOption) {
                    ...
                    dropdown.setAttribute('selected-item', QUEUE_TYPES[CONFIG.selectedQueue].name);
                }
        ...
        observeProfile() {
            ...
            this.observer = new MutationObserver(() => {
                ...
                const searchedProfile = Array.from(profileElements).find(el => el.getAttribute('is-searched') === 'true');
        ...
        async fetchStats(puuid) {
            ...
            const filteredGames = CONFIG.selectedQueue === 'all' 
                ? data.games.games 
                : data.games.games.filter(game => game.queueId === QUEUE_TYPES[CONFIG.selectedQueue].id);

            const stats = filteredGames.reduce((acc, game) => {
        ...
        createStatsContainer() {
            ...
            const existingContainer = document.getElementById('profile-win-loss-container');
